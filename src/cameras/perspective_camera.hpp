...
namespace RT_ISICG
{
	class PerspectiveCamera : public BaseCamera
	{
	  public:
		...
		{
			Vec3f viewPortPosition = _viewportTopLeftCorner + ( p_sx * _viewportU ) - ( p_sy * _viewportV );
			Vec3f rayDir		   = normalize( viewPortPosition - getPosition() ); // direction du rayon
			return Ray( getPosition(), rayDir );	// lancer du rayon
		}
